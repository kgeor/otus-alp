# DNS
Первые два подпункта для наглядности описывают ручное приведение всей системы к требуемому состоянию (конфигурация и зоны DNS взяты из https://github.com/erlong15/vagrant-bind), но при запуске стенда сейчас, Ansible сразу сконфигурирует все хосты так, как требуется по заданию, поэтому никакой доп. конфигурации не нужно, сразу можно переходить к подпункту Проверка. 
